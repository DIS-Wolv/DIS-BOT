...
# liste emoji
emote = [secrets.DIS_EMOTE_ID, secrets.CDG_EMOTE_ID, secrets.CDE_EMOTE_ID, secrets.MED_EMOTE_ID, secrets.MINI_EMOTE_ID]
...
async def startloop(m=45):
    ...
    reponse = ":white_check_mark: Redémarage des évènements répétitifs a : " + str(datetime.now().minute)
...
async def on_member_join(member):
    ...
    logchannel = bot.get_channel(secrets.LOG_CHANNEL_ID)
    await member.send(new_member_msg())  # mp le nouveaux avec le message dans new_member_msg.txt
    ...
    role = discord.utils.get(server.roles, id=role_id)  # defini l'objet role avec les bon attribue
...
async def on_message(message):
    ...
    if str(message.channel.type) == 'text':
        ...
        print(message.channel.name, ">", message.author.display_name, ":", str(message.content))  # affiche le message dans la console content
    else:
        #print(type(message.channel.type),message.channel.type, "!= 'text'")
        print("DM >", message.author.display_name, ":", message.content)  # affiche le message dans la console
    if message.channel.id == secrets.CHANNEL_ID:
        # print(message.channel.id, secrets.CHANNEL_ID)
        if testmsg.mention(message.content) == 1:  # si le message contient la mention
            jour = testmsg.jour(message.content)  # regarde si il y a un jour dans le message
            # print("Ajout des reaction sur le message de ", jour)
            if jour != 0:  # si oui
                # print("REACTION!!!!!!!")
                await message.add_reaction('<:cdg:' + str(secrets.CDG_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
                await message.add_reaction('<:cde:' + str(secrets.CDE_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
                await message.add_reaction('<:medecin:' + str(secrets.MED_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
                await message.add_reaction('<:mg:' + str(secrets.MINI_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
                await message.add_reaction('<:DIS:' + str(secrets.DIS_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
                await message.add_reaction(str('❌'))  # ajoute les réactions d'inscriptions

    elif message.channel.id == secrets.LOG_CHANNEL_ID and message.author.id != int(secrets.CLIENT_ID): #COMMANDE
        ...
        if test != -1:  # si le message comptient "update"
            ...
            await log.send("Début de la mise à jour des messages")  # envoie un message dans les logs

            await appelMessage()  # appel la fonction de creation de message

        test = testmsg.mots(message.content, "planning")  # si le message comptient "planning"
        if test != -1:
            reponse = ""
            await message.channel.send("Vérification du planning ...")  # envoie un message
        ...
        test = testmsg.mots(message.content, "clear")
        if test != -1:
            jour = testmsg.jour(message.content)  # regarde si il y a un jour dans le message
        ...
        test = testmsg.mots(message.content, "addUser")
        if test == 0:
            msg = message.content
            msg = msg.split(' ')
            if len(msg) >= 2:
            ...
            reponse = ":white_check_mark: Ajout de \"" + msg[1] + "\" avec l'id ``" + msg[2] + "``"
            await message.channel.send(reponse)  # envoie un message
        ...
        if test != -1:
            ...
            if loopS < 00:
                ...
                print(msg)
                if len(msg) > 2:
                    reponse = ":white_check_mark: Redémarage des évènements répétitifs a : " + msg[1]
                    await message.channel.send(reponse)  # envoie un message
        ...
        if test != -1:
            print("Arret du bot manuel le : ", datetime.now().day, "/", datetime.now().month, "/", datetime.now().year, 'à', datetime.now().hour, "h", datetime.now().minute, "par", message.author.display_name)
            loop.cancel()
            await message.channel.send("Arret du Bot")
            await bot.logout()

        test = testmsg.mots(message.content, "statut")
        if test == 0:
            ...
            reponse = ":white_check_mark: changement du statut pour ``Joue à " + statut + "``"
            await message.channel.send(reponse)  # envoie un message

        test = testmsg.mots(message.content, "live")
        if test == 0:
            reponse = "Changement du statut"
            await message.channel.send(reponse)  # envoie un message
            msg = message.content
            ...
            reponse = ":white_check_mark: changement du statut pour ``Stream " + statut + "``"
            await message.channel.send(reponse)  # envoie un message

        test = testmsg.mots(message.content, "acceuil")
        if test == 0:
        ...
        test = testmsg.mots(message.content, "randomstatus")  # si le message comptient "RandomStatus"
        if test != -1:
    ...
    test0 = testmsg.mots(message.content, "sondage")
    test1 = testmsg.mots(message.content, "réaction")
    test2 = testmsg.mots(message.content, "dlc")
    # print(test0, test1, test2)
    if test0 != -1 and test1 != -1 and test2 != -1:
        # print("Ajout des reactionsDLC")
        await message.add_reaction('<:karts:' + str(secrets.KART_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:heli:' + str(secrets.HELI_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:marksmen:' + str(secrets.MARK_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:apex:' + str(secrets.APEX_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:jets:' + str(secrets.JETS_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:tanks:' + str(secrets.TANKS_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:lawOfWar:' + str(secrets.LOW_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:globalMobilization:' + str(secrets.GBMOB_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:contact:' + str(secrets.CONTACT_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:PrairieFire:' + str(secrets.PRAIRIEFIRE_DLC_EMOTE_ID) + '>')  # ajoute les réactions d'inscriptions
        await message.add_reaction('<:WesternSahara:' + str(secrets.WESTERNSAHARA_DLC_EMOTE_ID) + '>')


# quand une réaction est ajoutée
@bot.event
async def on_raw_reaction_add(payload):
    guild = bot.get_guild(payload.guild_id)  # recupère le pool d'utilisateurs qui on réagi
    user = await guild.fetch_member(payload.user_id)  # recupère l'utilisateur qui a réagi

    channel = await bot.fetch_channel(payload.channel_id)  # recupère le channel
    message = await channel.fetch_message(payload.message_id)  # récupère le message en question
    #print(payload.emoji)
    ...
    if payload.channel_id == secrets.CHANNEL_ID and str(user.id) != secrets.CLIENT_ID:  # si la réaction est dans le channel souhaité
        ...
        jour = testmsg.jour(message.content)  # recupère le jour du message
        ...
        if str(payload.emoji) == str('❌') and jour != 0:
            ...
            if statut == 0:
                ...
                err = user.display_name + " (Id = `" + str(user.id) + "`) n'a pas pu etre désinscrit"
            elif statut == -1:
                err = ""
            else:
...
async def appelInscription(user, emote, jour):
    ...
    if statut == 0:  # deja inscrit
        err = ""
    elif statut == 1:  # succes
        err = ""
    elif statut == 2:  # Utilisateur pas dans la liste
        err = user.display_name + " (Id = `" + str(user.id) + "`) n'a pas pu être inscrit."
        print("Ajout de l'utilisateur : ", user.display_name)
        inscription.addUser(user.display_name, str(user.id))
        print("utilisateur ajouté")
        err += "\n:white_check_mark: Ajout de \"" + user.display_name + "\" avec l'id ``" + str(user.id) + "``"
    else:
        err = "ERREUR Inscription :" + user.display_name + " id `" + str(user.id) + "`"
...
async def appelDLC(user, emote, state):
    ...
    if statut == 1:  # succes
        ...
        err = user.display_name + " (Id = `" + str(user.id) + "`) n'es pas dans la fiche Technique."
        inscription.addUser(user.display_name, str(user.id))
        err += "\n:white_check_mark: Ajout de \"" + user.display_name + "\" avec l'id ``" + str(user.id) + "``"
    else:
...
async def on_raw_reaction_remove(payload):
    guild = bot.get_guild(payload.guild_id)  # recupère le pool d'utilisateurs qui on réagi
    user = await guild.fetch_member(payload.user_id)  # recupère l'utilisateur qui a modifié sa réaction
...
async def updateMessage(msg):
    ...
    jour = testmsg.jour(msg.content)  # récupère le jours du message
    ...
    if newmsg != 0:
        ...
        # print(jour)
        await SetActivity(str(inscription.missionName(jour) + ' ' + jourNom[jour] + " soir"))
    ...
    if hour <= 2:  # si l'heure est inférieure ou égale a 2
        ...
        if inscription.missionName(day) != '':
            ...
            if inscription.missionName(day) == 'ODD de la semaine !':
                ...
                async for message in channel.history(limit=7):  # pour les 7 derniers messages
                    jourMsg = testmsg.jour(message.content)  # recupère le jour du message
...
async def appelMessage():
    ...
    async for msg in channel.history(limit=7):  # pour les 7 dernier message
        ...
        if str(msg.author.id) == str(secrets.CLIENT_ID):  # si il on été envoyer par le bot
            jour = testmsg.jour(msg.content)  # récupère le jours du message
    ...
    for i in range(-1, 2):  # sur 3 jours
        ...
        if msg != 0:  # si il y a un message a envoyer = missions
            ...
            async for message in channel.history(limit=7):  # pour les 7 derniers messages
                jourMsg = testmsg.jour(message.content)  # recupère le jour du message s'il y a
            ...
            if statut:  # si le statut a changé
                ...
                await SetActivity(inscription.missionName(target) + ' ' + jourNom[target] + " soir")

# si la boucle est fini
@loop.after_loop
async def Erreur():
    print(datetime.now().day, "/", datetime.now().month, "/", datetime.now().year, 'à', datetime.now().hour, "h",
          datetime.now().minute)
